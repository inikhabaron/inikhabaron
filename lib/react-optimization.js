...
export const MemoComponent = memo(
  ({ children, debug = false }) => {
    if (debug) {
      const logger = (typeof window === 'undefined') ? null : null;
      // Use console.debug in client for development debug messages
      if (debug && typeof console !== 'undefined') console.debug('Rendering memoized component');
    }
    return children;
  },
  (prevProps, nextProps) => {
