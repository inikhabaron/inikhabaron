...
import { timeAsync } from './perf/perfLog.js';
...
function connect() {
  ...
  // Every call to connect() means getClientPromise() found no cached promise
  // on `global` — i.e. a genuinely NEW MongoClient + connection pool, not a
  // reused one. Labeled distinctly from the cache-hit path below so
  // production logs directly answer "is a new client being created per
  // request, or is the pool being reused".
  const promise = timeAsync('MongoClient.connect() [NEW CLIENT]', () => client.connect());

  // A *rejected* promise cached on `global` is just as sticky as a fulfilled
  // one, so a single transient failure to connect — a cold function racing a
  // slow Atlas handshake against the deliberately short
  // serverSelectionTimeoutMS above — used to make every subsequent request on
  // that warm instance fail with the same replayed error, for the life of the
  // instance. Evicting the cache on rejection means the next request gets a
  // fresh attempt instead. (Attaching the handler here also keeps the
...
function getClientPromise() {
  ...
  } else if (process.env.PERF_LOGS !== 'false') {
    console.log('[PERF] MongoClient: reused cached client (0ms, no new connection)');
  }
  return global._mongoClientPromise;
}

export async function getDatabase() {
  // Read through the accessor rather than a module-scoped copy, so a request
  // that arrives after an evicted failure actually picks up the retry.
...
export async function getCollection(collectionName) {
  return timeAsync(`getCollection("${collectionName}")`, async () => {
