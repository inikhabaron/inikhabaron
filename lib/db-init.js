...
export async function createIndexesFromDb(db) {
  try {
    ...
    // Compound index covering category-filtered list queries: WHERE status='published' AND category=X ORDER BY publishedAt
    await safeCreateIndex('news', { status: 1, category: 1, publishedAt: -1 }, undefined, '(status, category, publishedAt)');
