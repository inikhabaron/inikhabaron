...
export function canEditReel(user, reel) {
  if (checkRole(user, ['admin'])) return true;
  if (checkRole(user, ['editor'])) return true;
  if (checkRole(user, ['reporter']) && reel.reporterId === user.id) {
    return reel.status === 'draft';
  }
  return false;
}

export function canPublishReel(user) {
  return checkRole(user, ['admin']);
}

// Gates moderation-only fields (isSensitive/reportStatus) — separate from
// canEditReel's draft-ownership gate, since these are moderation actions,
// not content edits. Mirrors canModerateComments' role set exactly.
export function canModerateReel(user) {
  return checkRole(user, ['admin', 'editor', 'reporter']);
}