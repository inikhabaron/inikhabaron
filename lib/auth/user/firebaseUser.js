...
export async function getFirebaseUserFromToken(idToken) {
  ...
  try {
    ...
    const setOnInsert = {
      ...
      // 'reader' is the canonical non-staff role — the name the admin Users
      // form offers and the one /api/users/sync assigns. This wrote 'user',
      // so the same concept had two names depending on which sign-in path
      // created the account. No authorization ever depended on either value
      // (every role check whitelists admin/editor/reporter), which is why the
