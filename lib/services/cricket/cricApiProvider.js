...
// `match_scorecard`, not `match_info`: verified against live responses,
// `match_info` never returns a `scorecard` field at all, so the match page's
// entire Scorecard section rendered empty no matter what the mapping did.
// `match_scorecard` returns every field `match_info` does that this module
// reads (id/name/matchType/status/venue/date/dateTimeGMT/teams/teamInfo/
// score/toss*/series_id/matchStarted/matchEnded) *plus* `scorecard`, so it
// is a strict superset here and costs the same single request.
//
