...
export const getArticle = cache(async function getArticle(id) {
  ...
  try {
    ...
    const article = serialize(doc);
    // authorAvatar stored on the article is a snapshot from when it was
    // created/backfilled — resolve the author's current avatar live here so
    // a later profile photo update shows up immediately.
    if (article.authorId) {
      const users = await getCollection('users');
      const authorDoc = await users.findOne(
        { id: article.authorId },
        { projection: { avatar: 1 } }
      );
      article.authorAvatar = authorDoc?.avatar || null;
    }
    return article;
  } catch (err) {
 ...
 */
export const getLatestArticles = cache(async function getLatestArticles({ limit = 12, category, excludeId } = {}) {
  try {
    const news = await getCollection('news');
