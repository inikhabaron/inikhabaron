...
// Videos live under a different resource_type; destroy() defaults to 'image'
// and silently reports "not found" for a video public_id, so the type has to
// be passed explicitly for anything the media maintenance tool deletes.
export async function deleteAsset(publicId, resourceType = 'image') {
  try {
    ...
    // destroy() resolves with { result: 'not found' } instead of throwing when
    // the asset is already gone — treat anything other than 'ok' as a failure
    // so the caller never records a deletion that did not happen.
    if (result?.result !== 'ok') {
      return { success: false, error: result?.result || 'Unknown destroy result', result };
    }
...
// Paginated listing of every asset of one resource type. Used only by the
// admin maintenance scan.
export async function listAssets({ resourceType = 'image', maxResults = 500, nextCursor } = {}) {
