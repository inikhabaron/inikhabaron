...
//   authors: [{ name, image, bio }, ...]
// Legacy shape (every article created before per-article author photos):
//   authorName / author / writer / byline  +  authorAvatar
//
// `bio` is optional and additive — articles saved before "About Author"
// existed simply have no `bio` on any author entry, and every reader of
// getArticleAuthors() already treats a falsy field as "nothing to show".
//
...
// A bio is a byline blurb, not an essay — long enough for a real one or two
// sentence intro, short enough that it can't be used to dump an article's
// worth of text under a byline. Enforced in the form (character counter) and
// again server-side (silently truncated, not rejected — a crafted or pasted
// payload shouldn't fail the whole save over this).
export const MAX_BIO_LENGTH = 800;

/**
 * Display-ready byline for an article, oldest-shape-tolerant.
 * @returns {Array<{ name: string, image: string|null, bio: string|null }>} possibly empty
 */
export function getArticleAuthors(article) {
  ...
  if (Array.isArray(article.authors) && article.authors.length) {
    const authors = article.authors
      .map((a) => ({
        ...
        bio: typeof a?.bio === 'string' && a.bio.trim() ? a.bio.trim() : null,
      }))
      .filter((a) => a.name);
  ...
  return [{
    ...
    // Legacy articles predate per-author bios entirely — there is no field
    // to fall back to, so this shape never has one.
    bio: null,
  }];
}
    ...
    .map((a) => ({
      ...
      bio:
        typeof a?.bio === 'string' && a.bio.trim()
          ? a.bio.trim().slice(0, MAX_BIO_LENGTH)
          : null,
    }))
    .filter((a) => a.name)
    .slice(0, MAX_AUTHORS);
